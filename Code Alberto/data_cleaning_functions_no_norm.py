        ...
        for patient in range(N_patients):
            ...
            if np.prod(np.isnan(Y)):
                ...
                if np.sum(np.isnan(Y)):
                    ...
                    VS_col_new = np.append(VS_col_new, Y)
        data_set_new[:,ii] = VS_col_new
        ii = ii + 1
        bar1.next()
    bar1.finish()
    return data_set_new

def tests_cleaning(data_set, tests, h_obs, test_min_in, test_max_in):
    print('-- tests features --')
    N_patients = np.array(data_set.shape[0]/h_obs).astype(int)
    ii = 0 
    if np.logical_not(np.any(test_min_in)):
        test_min = np.nanmin(data_set[tests].loc[:], axis = 0)
    else:
        test_min = test_min_in
    if np.logical_not(np.any(test_max_in)):
        test_max = np.nanmax(data_set[tests].loc[:], axis = 0)
    else:
        test_max = test_max_in  
    ...
    for test in tests:
        ...
        for idx in range(test_col.shape[0]):
            if np.logical_not(np.isnan(test_col[idx])):
                test_col_new[idx] = 1+(test_col[idx] - test_min[ii])/(test_max[ii]-test_min[ii])
            
        data_set_new[:,ii] = test_col_new
        ii = ii + 1
        bar2.next()
    bar2.finish()
    return data_set_new, test_min, test_max
