...
tests_Set.to_csv('data/train_features_clean_all_3.csv', header=True, index=False)
print("NaN still inside per columns: ", np.sum(np.isnan(np.array(tests_Set)), axis=0))
print(tests_Set.head())
print()
...
submSet.to_csv('data/test_features_clean_all_3.csv', header=col, index=False)
