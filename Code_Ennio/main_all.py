...
X_t1, X_val_t1, X_test_t1 = build_set(selected_houred_features_t1, train_size)

# task3
selected_houred_features_t3 = selected_houred_features_t1
...
for i in range(0, len(labels_target)):
    ...
    if features_selection:
        ...
        useful_features_augmented = \
            sum([[s for s in houred_features if feature in s] for feature in useful_features], []) \
            +[feature for feature in useful_features if feature in diff_features if use_diff]
...
for i in range(0, len(labels_target)):
    label_target = labels_target[i]
    Y_t1 = train_labels.loc[0:train_size - 1, label_target]
    Y_val_t1 = train_labels.loc[train_size:, label_target]
