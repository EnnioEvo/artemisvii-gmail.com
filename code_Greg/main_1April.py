...
#Subtask 1
Y_test_1=np.zeros((1000,11))
Y_test_1[0:1000,0]=X_test[0:1000,0]
print(Y_test_1.shape)
for ind1 in range(1,11):
    Y_train = np.array([data_set_y_train[0:1000, ind1]]).T
    print(Y_train.shape)
    clf = svm.SVC(gamma='scale',probability=False,class_weight='balanced')
    #clf = svm.SVC(probability=True,class_weight='balanced')
    clf.fit(X_train, Y_train)
    Y_test_1[:,ind1]=np.array([clf.decision_function(X_test)])
#print(Y_train)
#print(Y_train.shape)
print(Y_test_1)

#Subtask 2
Y_test_2=np.zeros((1000,2))
Y_test_2[0:1000,0]=X_test[0:1000,0]
print(Y_test_2.shape)
Y_train = np.array([data_set_y_train[0:1000, 11]]).T
print(Y_train.shape)
clf = svm.SVC(gamma='scale',probability=False,class_weight='balanced')
clf.fit(X_train, Y_train)
Y_test_2[0:1000,1]=np.array([clf.decision_function(X_test)])
#print(Y_train)
#print(Y_train.shape)
print(Y_test_2)

#Subtask 3
Y_test_3=np.zeros((1000,5))
Y_test_2[0:1000,0]=X_test[0:1000,0]
print(Y_test_3.shape)
for ind2 in range(1,5):
    Y_train = np.array([data_set_y_train[0:1000, ind2+11]]).T
    print(Y_train.shape)
    clf = svm.SVR(gamma='scale')
    clf.fit(X_train, Y_train)
    Y_test_3[0:1000,ind2]=np.array([clf.predict(X_test)])
#print(Y_train)
#print(Y_train.shape)
print(Y_test_3)
