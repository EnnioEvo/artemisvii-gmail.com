...
data_set_x_train_input = np.array(pd.read_csv("../Code_Alberto/data/train_features_clean_all_no_norm.csv"))
data_set_y_train_input = np.array(pd.read_csv("../data/train_labels.csv"))


#Concatenation of all the data for the 12 hours in data_set_x_train to obtain one single row for each patient
N = 227940
...
for ind1 in range(1,11):
    ...
    Y_testProb=clf.predict_proba(X_test)
    Y_temp_2=Y_ones-Y_testProb
...
clf=RandomForestClassifier(n_estimators=1000,class_weight="balanced_subsample")
clf.fit(X_train,np.ravel(Y_train))
Y_testProb2=clf.predict_proba(X_test)
Y_temp_4=Y_ones-Y_testProb2
