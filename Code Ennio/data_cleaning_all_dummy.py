...
def fill_patient_VS(VS_obs):
    if np.all(np.isnan(VS_obs)):
        raise ValueError("Full NaN observations cannot be imputed")
    new_VS = np.zeros([12,1])
    for i in range(12):
        if ~np.isnan(VS_obs[i]):
            new_VS[i] = VS_obs[i]
            ...
            while True:
                ...
                try:
                    if ~np.isnan(VS_obs[i+offset]):
                        new_VS[i] = VS_obs[i+offset]
                        break
                except IndexError:
                    pass
                finally:
    ...
    for VS in vital_signs:
        ...
        for i in range(N_patients):
            if np.all(np.isnan(VS_column[i * 12:(i + 1) * 12])):
                ...
                new_VS_column[i * 12:(i + 1) * 12] = fill_patient_VS(VS_column[i * 12:(i + 1) * 12])
    ...
    for test in tests:
        ...
        for i in range(N_patients):
            if np.all(np.isnan(test_column[i * 12:(i + 1) * 12])):
                ...
                day_mean = np.nanmean(test_column[i * 12:(i + 1) * 12])
                for j in range(i * 12, (i + 1) * 12):
                    if np.isnan(test_column[j]):
                        new_test_column[j] = day_mean
                        dummy_test_column[j] = 0
                    else:
                        new_test_column[j] = test_column[j]
